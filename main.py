...
class Quote:
    ...
    def get_quote(self):
        ...
        with open('quotes.txt', 'r') as f:
            quotes = f.readlines()
        self.prep_text(quotes[random.randint(0, len(quotes))])
...
# search_str = ''
# search_str += quote.nouns[0] if quote.nouns else ''
# search_str += ' ' + quote.verbs[0] if quote.verbs else ''
# search_str += ' ' + quote.adjectives[0] if quote.adjectives else ''
# folder = img_downloader.download(search_str)
# file_list = os.listdir(folder)
# path = file_list[random.randint(0, len(file_list))]
